...
async function normalizeRequest(
  ...
  const normalizedVector: NormalizedVector = [
    ...
    limit(
      ...
      normalizationConfig.amountVsAvgRatio,
    ),
    getHour(transaction.requested_at) / 23,
    getWeekDay(transaction.requested_at) / 6,
    last_transaction
      ? limit(
          getMinutes(last_transaction.timestamp),
          normalizationConfig.maxMinutes,
        )
      : -1,
    last_transaction
      ? limit(last_transaction.km_from_current, normalizationConfig.maxKm)
      : -1,
...
async function initHNWS() {
  index = new HierarchicalNSW("l2", 14);
  index.readIndexSync("references.dat");
  return index;
}

async function getIndex() {
  if (index) return index;
  return await initHNWS();
}
...
function countFraud(neighbors: Array<{ dist: number; label: string }>) {
  return neighbors.reduce(
    (count, neighbor) => count + (neighbor.label === "fraud" ? 1 : 0),
    0,
  );
}
...
async function performHNSW(normalizedVector: NormalizedVector) {
  const data = await getIndex();
  const result = data.searchKnn(normalizedVector, 3);
...
async function classifyTransaction(request: PayloadRequest) {
  ...
  const nhswSearch = await performHNSW(normalizedVector);

  const neighbors = nhswSearch.neighbors;
  ...
  const knn = refs
    ? neighbors
        .slice(0, 3)
        .map((index: number) => ({ dist: 0, label: refs[index]!.label }))
    : [];
